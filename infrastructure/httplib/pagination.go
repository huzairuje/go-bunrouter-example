...
func GetPaginationFromCtx(req bunrouter.Request) (*Query, error) {
	q := &Query{}
	if err := q.SetPage(req.URL.Query().Get("page")); err != nil {
		return nil, err
	}
	if err := q.SetSize(req.URL.Query().Get("size")); err != nil {
		return nil, err
	}
	q.SetOrderBy(req.URL.Query().Get("orderBy"))
	q.SetSortOrder(req.URL.Query().Get("sortOrder"))
